...
const _hospitals = JSON.parse(
  readFileSync(join(dataDir, "hospitals.json"), "utf-8"),
);
const _specialties = JSON.parse(
  readFileSync(join(dataDir, "specialties.json"), "utf-8"),
);
...
export function estimate({ specialty, planId }) {
  ...
  const opciones = _hospitals
    ...
    .sort(
      (a, b) => a.desglose.copago - b.desglose.copago || b.rating - a.rating,
    )
    .slice(0, 5);
